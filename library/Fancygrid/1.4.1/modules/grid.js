 ...
 */
  ...
  prepareConfigEdit: function(config){
    ...
    for(;i<iL;i++){
      ...
      if(column.editable && inluded === false){
        config._plugins.push(editPluginConfig);
  ...
  prepareConfigPaging: function(config, originalConfig){
    ...
    config.bbar.push({
      ...
      listeners: [{
        enter: function(field){
          me.paging.setPage(parseInt(field.getValue()) - 1);
        }
      }]
    });
    ...
    if(config.paging && Fancy.isArray(config.paging.pageSizeData)){
      var pageSizeData = config.paging.pageSizeData,
        sizes = [],
        i = 0,
      ...
      for(;i<iL;i++){
        ...
        if(config.paging.pageSize === pageSizeData[i]){
          value = i;
        }
      }
  ...
  prepareConfigSize: function(config, originalConfig){
    ...
    if(config.width === undefined){
      if(renderTo){
        ...
        //config.width = parseInt(el.css('width'));
...
Fancy.Mixin('Fancy.grid.mixin.Grid', {
  ...
  initTouch: function(){
    ...
    if(Fancy.isTouch && window.FastClick){
      if(me.panel){
        ...
        me.panel.el.addClass('fancy-touch');
      }
      else {
        FastClick.attach(me.el.dom);
        me.el.addClass('fancy-touch');
      }
    }
  },
  /*
  ...
  render: function(){
    ...
    if(me.panel){
      ...
      //panelBorderHeight = me.gridBorders[2];
    }
  ...
  setSides: function(){
    ...
    if(leftWidth === 0 && rightWidth === 0){
      if(me.panel){
        //Should be like this, but it is wrong for default
        //centerWidth = me.width - panelBorderWidth * 2 - borders[1] - borders[3];
        //centerWidth = me.width - panelBorderWidth * 2 - borders[1];
        centerWidth = me.width - gridBorders[1] - gridBorders[3] - panelBodyBorders[1] - panelBodyBorders[3];
      }
      else {
        //centerWidth = me.width - borders[1] - borders[3];
        centerWidth = me.width - gridWithoutPanelBorders[1] - gridWithoutPanelBorders[3];
      }
    }
    else if(rightWidth === 0){
      if(me.panel){
        //console.log(me.width, gridBorders[1], gridBorders[3], panelBodyBorders[1], panelBodyBorders[3]);
        //Should be like this, but it is wrong for default
        //centerWidth = me.width - leftWidth - panelBorderWidth * 2 - borders[1] - borders[3];
        //centerWidth = me.width - leftWidth - panelBorderWidth * 2 - borders[1];
        centerWidth = me.width - leftWidth - gridBorders[1] - gridBorders[3] - panelBodyBorders[1] - panelBodyBorders[3];
        //console.log(centerWidth);
      }
      else {
        //centerWidth = me.width - leftWidth - borders[1] - borders[3];
        centerWidth = me.width - leftWidth - gridWithoutPanelBorders[1] - gridWithoutPanelBorders[3];
      }
    }
    else if(leftWidth === 0){
      if(me.panel){
        //centerWidth = me.width - rightWidth - borders[1] - borders[3];
        centerWidth = me.width - rightWidth - gridBorders[1] - gridBorders[3] - panelBodyBorders[1] - panelBodyBorders[3];
      }
      else {
        //centerWidth = me.width - rightWidth - borders[1] - borders[3];
        centerWidth = me.width - rightWidth - gridWithoutPanelBorders[1] - gridWithoutPanelBorders[3];
      }
    }
    else if(me.width > leftWidth + centerWidth + rightWidth){
      if(me.panel){
        //centerWidth = me.width - leftWidth - rightWidth - borders[1];
        centerWidth = me.width - leftWidth - rightWidth - gridBorders[1] - gridBorders[3] - panelBodyBorders[1] - panelBodyBorders[3];
        //if(panelBorderWidth === 1){
        //centerWidth -= panelBorderWidth * 2;
        //}
      }
      else {
        //Here is place where in default theme, center width sould be less on 1
        //centerWidth = me.width - leftWidth - rightWidth - borders[1] - borders[3];
        centerWidth = me.width - leftWidth - rightWidth - gridWithoutPanelBorders[1] - gridWithoutPanelBorders[3];
      }
    }
    else{
      if(me.panel){
  ...
  getCellsViewHeight: function(){
    ...
    if(me.scroller.scrollBottomEl.hasClass('fancy-display-none')){}
    else {
  ...
  getCenterViewWidth: function(){
    //Realization could be reasone of bug
    var me = this,
  ...
  initTextSelection: function(){
    ...
    if(me.textSelection === false) {
      me.el.addClass('fancy-grid-unselectable');
  ...
  setWidth: function(width){
    ...
    if(me.panel){
      ...
      var gridWidth = width - panelBodyBorders[1] - panelBodyBorders[3];
      ...
      el.width(width - gridBorders[1] - gridBorders[3]);
      el.width(width);
    }
  ...
  setHeight: function(value, changePanelHeight){
    ...
    if(me.body){
      me.body.el.css('height', bodyHeight);
    }

    if(me.leftBody){
      me.leftBody.el.css('height', bodyHeight);
    }

    if(me.rightBody){
      me.rightBody.el.css('height', bodyHeight);
    }
  ...
  enableSelection: function(value){
    ...
    if(value !== undefined){
      me.selection.enabled = value;
      return;
    }

    if(me.selection){
      me.selection.enabled = true;
    }
  },
  hideColumn: function(side, index){
    ...
    switch(side){
      case 'left':
        ...
        leftHeader.el.css('width', parseInt(leftHeader.el.css('width')) - column.width);
        centerEl.css('left', parseInt(centerEl.css('left')) - column.width);
        centerEl.css('width', parseInt(centerEl.css('width')) + column.width);
        me.body.el.css('width', parseInt(me.body.el.css('width')) + column.width);
        me.header.el.css('width', parseInt(me.header.el.css('width')) + column.width);
        break;
      case 'right':
        rightEl.css('width', parseInt(rightEl.css('width')) - column.width);
        rightHeader.el.css('width', parseInt(rightHeader.el.css('width')) - column.width);
        centerEl.css('width', parseInt(centerEl.css('width')) + column.width);
        me.body.el.css('width', parseInt(me.body.el.css('width')) + column.width);
        me.header.el.css('width', parseInt(me.header.el.css('width')) + column.width);
        break;
...
Fancy.define('Fancy.grid.plugin.Scroller', {
  ...
  render: function(){
    ...
    if(w.panel){
      if(w.panelBorderWidth === 0){
        //bottomScrollEl.css('bottom', '1px');
      }
      //bottomScrollEl.css('bottom', Math.floor(w.panelBorderWidth/2));
    }
    else{
      //bottomScrollEl.css('bottom', '1px');
    }
  ...
  scroll: function(y, x){
    ...
    w.leftBody.scroll(y, x);
    scrollInfo = w.body.scroll(y, x);
    w.rightBody.scroll(y, x);
...
Fancy.define('Fancy.grid.plugin.ColumnResizer', {
  ...
  onCellMouseMove: function(grid, o){
    ...
    if(!isInTrigger && !isInTriggerImage && offsetX < 7 && o.index !== 0){
      if(me.isColumnResizable(o, true)){
        if( Fancy.get(e.target).closest('.fancy-field').hasClass('fancy-field') === false ){
          me.addCellResizeCls(o.cell);
        }
      }
    }
    else if((_width - offsetX)<7){
  ...
  moveResizeEls: function(e){
    ...
    if(cellWidth < w.minCellWidth){
      cellWidth = w.minCellWidth;
    }
  ...
  renderSparkLine: function(i, rowIndex, type){
    ...
    for(;j<jL;j++){
      ...
      if(Fancy.nojQuery){
        ...
        $(innerDom).sparkline(value, sparkConfig);
      }
    }
  },
  /*
...
Fancy.define('Fancy.grid.Header', {
  ...
  insertCell: function(index, column){
    ...
    me.el.css('width', parseInt(me.el.css('width')) + column.width);
  },
  setAlign: function(){
    var me = this,
      w = me.widget,
  ...
  removeCell: function(orderIndex){
    ...
    if(me.side !== 'center'){
      me.el.css('width', parseInt(me.el.css('width')) - cellWidth);
    }
  }
});